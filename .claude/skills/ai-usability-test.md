...
> | `frontend/tests/ut/ut-heuristics.mjs` | 결정론적 Nielsen 검출기(above-fold CTA·destructive-confirm·form-error-recovery·keyboard-reachable·loading-feedback·control-consistency) — 시나리오 `run()`이 호출해 표준 `errorType` 방출. "AI가 눈으로 판단" 대신 관찰 조건을 코드로 검사. 계약: `UT_HEURISTIC_RUBRIC.md` |
> | `frontend/tests/ut/UT_HEURISTIC_RUBRIC.md` | **판정 계약** — `errorType`→휴리스틱→severity→신뢰도 규칙표. 게이트는 결정론 규칙에만 걸고 LLM 자유 라벨은 advisory 로 분리하는 근거 |
...
> 분류는 `UT_HEURISTIC_RUBRIC.md` 계약을 따른다 — `errorType` 이 규칙표에 등재되면 **결정론 규칙(provenance=rule)** 으로 게이트에 반영되고(severity 는 규칙표가 권위), 규칙표에 없이 `severityHint`/`heuristic` 만 붙은(=LLM 사후 라벨) findings 는 **advisory(게이트 제외, 사람 검토)** 로 분리된다. 옛 "무조건 N1/S3" 기본값은 폐기됐다(근거 없는 게이트 방지).
...
> ⚠️ **AI 개선안은 "가설"이다 — 검증 전 배포 금지.** 생성형 AI가 낸 UX/CRO 개선안은 상당수가 전환율에 중립·부정적일 수 있다.
> 자유 생성 대신 `UT_HEURISTIC_RUBRIC.md` §4 **패턴 카탈로그**(위반 휴리스틱 → 검증된 수정 패턴)에서 선택·적응하고,
> 모든 안은 A/B 또는 실측 UT(`ut-calibrate`)로 검증할 **가설**로 표기한다. 자동 반영·무검증 배포 금지.

각 Critical/Major 결함마다 **레이아웃 안 2종** 제시 (카탈로그 기반):
- **안 A**: 기존 패턴 유지, 최소 변경
- **안 B**: 사용자 흐름 재설계 (권장)
...
| `advisory` | AI 판단·미분류 findings 수 (규칙표 미등재 → 게이트 제외, 사람 검토용) | 게이트 비권장(참고용) |
